...
> 本条覆盖统一规则 L0-8 中「当前指定：`DeepSeek-V4-Flash[free]`」的本仓子代理模型选择；其余并发与自包含要求不变。

- 本仓所有 Trellis / Codex 子代理派发**只能使用 `glm-5.3-flash`**。
- 派发前必须核对当前平台**实际可用模型列表**；只有列表中存在精确模型名 `glm-5.3-flash` 时才允许派发。
- 用户明确指定其他子代理模型时，先核对列表：若该模型不在可用列表中，**立即停止派发**，不得静默改成继承模型或近似模型；向用户报告可用列表后，改用 `glm-5.3-flash` 重新派发须先获确认。
- 用户未明确指定子代理模型时，**禁止默认继承主模型**；必须停下询问，并列出当前平台可用模型列表，默认推荐 `glm-5.3-flash`。
- 派发后必须核对子代理通知或执行轨迹中的实际模型；一旦发现模型不是 `glm-5.3-flash`，立即停止该子代理工作，不采信其写入结果，再按上一条处理。
- 若 `glm-5.3-flash` 不可用，只能停止并询问，不得降级到旗舰模型、其他免费模型或主模型。
