...
const HOST_ORDER = ['sillytavern', 'luker', 'puretavern', 'tauritavern'];
...
export function getRegistryHostLabel(host) {
    return HOST_LABELS[host];
}
export function getRegistryCapabilityLabel(capability) {
    return CAPABILITY_LABELS[capability];
}
export function getRegistryHostLevelLabel(level) {
    switch (level) {
...
export function getRegistrySourceLabel(source) {
    return SOURCE_LABELS[source];
}
export function getRegistryConflictKindLabel(kind) {
    return CONFLICT_KIND_LABELS[kind];
}
/** 插件注册表版块（L12 只读发现层）。清单 → 冲突 → 详情三视图，
 * 全部由主入口的 `data-action="registry-*"` / `data-tab="registry"` 交互驱动。
 */
export function renderRegistryWorkbench(state, options = { isAdmin: false }) {
    if (state.error) {
        return `
...
function renderRegistryListView(snapshot, isAdmin, refreshing) {
...
function renderRegistryRecordTable(records, conflicts) {
    if (records.length === 0) {
        return '<div class="authority-empty">当前没有发现任何扩展目录。</div>';
    }
    const conflictCountByExtension = new Map();
...
function renderRegistryRecordView(record, state) {
...
function renderRegistryDependencyMatrix(dependencies) {
...
function renderRegistryCrossHostGrid(record) {
    return `
...
function renderRegistryConflictGroups(conflicts) {
...
function renderRegistryConflictRow(conflict, severity) {
    return `
...
function renderRegistryDiagnostics(diagnostics) {
...
//# sourceMappingURL=registry-workbench.js.map