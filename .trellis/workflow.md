...
Model gate before any dispatch: use only `glm-5.3-flash`, and only after verifying that exact name exists in the current platform's available sub-agent model list. If the user names another model, verify it first; if it is absent or mismatched, stop dispatch immediately and report the available list instead of inheriting or approximating a model. If the user did not specify a model, stop and ask which model to use, listing the available options with `glm-5.3-flash` as the default recommendation. After dispatch, verify the actual model from the sub-agent notification or trace; on any mismatch, stop the sub-agent and do not trust its writes.
